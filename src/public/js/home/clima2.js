...
$(document).ready(function () {
	...
	function error() {
		console.log("Error obteniendo la ubicacion");
	}
	const getClima = async (lat,long) => {
		const resp = await $.getJSON(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=c6fe4fe979e5916212a04c5c40d7b430&units=metric&lang=es`)
		display(resp);
	}
	...
	function display(data) {
		...
		//console.log(city,temp,desc,date);
		updateSummaryText(desc);
		var months = [
			"Enero",
			"Febrero",
			"Marzo",
...
function updateSummaryText(descrip) {
	summary.html(descrip.name);
	TweenMax.fromTo(
		summary,
		1.5,
		{ x: 30 },
